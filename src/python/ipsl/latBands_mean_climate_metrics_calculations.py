...
def compute_metrics_latBands(var,dm_glb,do_glb):
    cdms.setAutoBounds('on')
    metrics_dictionary = {}
    
    domains = ['Arctic','SubTropicsNorth','TROPICS','SubTropicsSouth']
    ...
    for dom in domains:
        ...
        if dom == 'Arctic':
            dm = dm_glb(latitude = (60.,90))
            do = do_glb(latitude = (60.,90))
        if dom == 'SubTropicsNorth':
            dm = dm_glb(latitude = (30.,60))
            do = do_glb(latitude = (30.,60))
        if dom == 'SubTropicsSouth':
