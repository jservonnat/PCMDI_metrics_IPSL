...
def compute_metrics_ExtInfo(var,dm_glb,do_glb):
    
    # Provides the PCMDI metrics with more info ; IPSL
    ...
    for dom in domains:
        ...
        if dom == 'NHEX':
            ...
            GeographicalBounds='-180.00E_180.00E_30.00N_90.00N'
        if dom == 'SHEX':
            dm = dm_glb(latitude = (-90.,-30))
            do = do_glb(latitude = (-90.,-30))
            GeographicalBounds='-180.00E_180.00E_90.00S_30.00S'
        if dom == 'TROPICS':
            dm = dm_glb(latitude = (-30.,30))
            do = do_glb(latitude = (-30.,30))
            GeographicalBounds='-180.00E_180.00E_30.00S_30.00N'
        if dom == 'GLB':
            GeographicalBounds='-180.00E_180.00E_90.00S_90.00N'
        ...
        for m in ['rms_xyt','rms_xy','bias_xy','cor_xyt','mae_xy']:
            ...
            if m == 'rms_xyt': metrics_dictionary[m + '_ann_' + dom] = {
                                                                        'MetricName':m, 'Region':dom, 'DataType':'ann',
                                                                        'GeographicalBounds':GeographicalBounds,
                                                                        'Result':format(rms_xyt*conv,sig_digits),
                                                                        }
            #if m == 'rms_xy': metrics_dictionary[m + '_ann_' + dom] =  format(rms_xy*conv,sig_digits)
            if m == 'rms_xy': metrics_dictionary[m + '_ann_' + dom] = {
                                                                        'MetricName':m, 'Region':dom, 'DataType':'ann',
                                                                        'GeographicalBounds':GeographicalBounds,
                                                                        'Result':format(rms_xy*conv,sig_digits),
                                                                        }
            #if m == 'bias_xy': metrics_dictionary[m + '_ann_' + dom] = format(bias_xy*conv,sig_digits)
            if m == 'bias_xy': metrics_dictionary[m + '_ann_' + dom] = {
                                                                        'MetricName':m, 'Region':dom, 'DataType':'ann',
                                                                        'GeographicalBounds':GeographicalBounds,
                                                                        'Result':format(bias_xy*conv,sig_digits),
                                                                        }
            #if m == 'mae_xy': metrics_dictionary[m + '_ann_' + dom] = format(mae_xy*conv,sig_digits)
            if m == 'mae_xy': metrics_dictionary[m + '_ann_' + dom] = {
                                                                        'MetricName':m, 'Region':dom, 'DataType':'ann',
                                                                        'GeographicalBounds':GeographicalBounds,
                                                                        'Result':format(mae_xy*conv,sig_digits),
                                                                        }
            #if m == 'cor_xyt': metrics_dictionary[m + '_ann_' + dom] = format(cor_xyt,'.2f')
            if m == 'cor_xyt': metrics_dictionary[m + '_ann_' + dom] = {
                                                                        'MetricName':m, 'Region':dom, 'DataType':'ann',
                                                                        'GeographicalBounds':GeographicalBounds,
                                                                        'Result':format(cor_xyt*conv,sig_digits),
                                                                        }
        ...
        for sea in ['djf','mam','jja','son']:
            ...
            metrics_dictionary['bias_xy_' + sea + '_' + dom] = {
                                                    'MetricName':'bias_xy', 'Region':dom, 'DataType':sea,
                                                    'GeographicalBounds':GeographicalBounds,
                                                    'Result':format(bias_sea*conv,sig_digits),
                                                    }
            #metrics_dictionary['rms_xy_' + sea + '_' + dom] = format(rms_sea*conv,sig_digits)
            metrics_dictionary['rms_xy_' + sea + '_' + dom] = {
                                                    'MetricName':'rms_xy', 'Region':dom, 'DataType':sea,
                                                    'GeographicalBounds':GeographicalBounds,
                                                    'Result':format(rms_sea*conv,sig_digits),
                                                    }
            #metrics_dictionary['cor_xy_' + sea + '_' + dom] = format(cor_sea,'.2f')
            metrics_dictionary['cor_xy_' + sea + '_' + dom] = {
                                                    'MetricName':'cor_xy', 'Region':dom, 'DataType':sea,
                                                    'GeographicalBounds':GeographicalBounds,
                                                    'Result':format(cor_sea*conv,sig_digits),
                                                    }
            #metrics_dictionary['mae_xy_' + sea + '_' + dom] = format(mae_sea*conv,sig_digits)
            metrics_dictionary['mae_xy_' + sea + '_' + dom] = {
                                                    'MetricName':'mae_xy', 'Region':dom, 'DataType':sea,
                                                    'GeographicalBounds':GeographicalBounds,
                                                    'Result':format(mae_sea*conv,sig_digits),
                                                    }

    for dum in metrics_dictionary:
        metrics_dictionary[dum].update( {
                                   'Variable':var,
                                   'p_value':'N/A',
                                   '95signifThreshold':'N/A',
                                   'MetricFreeSpace':'PCMDI_Metrics',
                                   'MetricContactExpert':'jerome.servonnat@lsce.ipsl.fr',
                                   'MetricReferencePaper':'Gleckler et al. (2009)',
                                   'MetricTrackingDate':datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                                  } )
