...
for model_version in parameters.model_versions:   # LOOP THROUGH DIFFERENT MODEL VERSIONS OBTAINED FROM input_model_data.py
  ...
  sft.targetGrid = None
  # START IPSL Modif
  sft.realization = parameters.realization
  # END IPSL Modif
  applyCustomKeys(sft,parameters.custom_keys,"sftlf")
  try:
    # START IPSL Modif
    #sft_mask = pcmdi_metrics.io.base.Base(parameters.MaskFilePath,parameters.MaskFileName)
    #sftlf[model_version] = {"raw":sft_mask.get("mask")}
    #sftlf[model_version]["filename"]=os.path.basename(sft_mask())
    #sftlf[model_version]["md5"]=sft_mask.hash()
    # END IPSL Modif
    ...
    for region in regions_dict[var]:
      ...
      for ref in refs:
        ...
        try:
              ...
              while success:
                  ...
                  try:
                     if level is None:
                       ...
                       levelVarInFile = varInFile+tmp_level
		  ...
		  # START IPSL Modif
		  # - Change values greater than 10^20 with nan
		  #dm[abs(dm) > dm.mimissin_value] = dm.fill_value
		  #do[abs(do) > 10^21] = do.fill_value
		  # END IPSL Modif
		      ...
		      # START IPSL Modif
                      ...
                      #        }
                      descr = {
                              "ModelActivity":parameters.ModelActivity,
                              "ModellingGroup":parameters.ModellingGroup,
                              "Model":model_version,
                              "MIPTable":obs_dic[var][obs_dic[var][ref]]["CMIP_CMOR_TABLE"],
                              "Center":parameters.Center,
                              "Login":parameters.Login,
                              "ModelFreeSpace":parameters.ModelFreeSpace,
                              "Experiment":parameters.Experiment,
                              "SimTrackingDate":parameters.SimTrackingDate
                              }
